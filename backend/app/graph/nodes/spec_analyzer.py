...
def spec_analyzer_node(state: ProcureOSState) -> Dict[str, Any]:
    ...
    Parses technical BOM file into structured line items using Document Parser MCP.
    """
    logger.info("Executing [Node 1: Spec & BOM Analyzer Agent]")
    bom_path = state.get("bom_file_path", "")
    
    line_items = doc_parser_mcp_server.extract_bom(bom_path)
    total_budget_est = sum(item["quantity"] * item["target_price"] for item in line_items)
    
    log_entry = f"Spec Analyzer successfully parsed {len(line_items)} BOM line items. Estimated Budget Ceiling: ${total_budget_est:,.2f}"
    logger.info(log_entry)
    
    return {
        "line_items": line_items,
        "budget_ceiling": total_budget_est * 1.15, # 15% ceiling flexibility guardrail
